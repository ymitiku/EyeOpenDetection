...
def eye_open_webcam_demo(model_path):
    eyeclosed_model = EyeClosedModel((32,32,1))
    ...
    while cap.isOpened():
        ...
        for i,face in enumerate(faces):
            ...
            left_eye = cv2.resize(left_eye,(32,32))
            right_eye = get_right_eye(face_img,dlib_points)
            right_eye = cv2.resize(right_eye,(32,32))
            left_eye = left_eye.astype(np.float32)/255
            right_eye = right_eye.astype(np.float32)/255
            left_prediction = model.predict(left_eye.reshape(-1,32,32,1))[1]
            right_prediction = model.predict(right_eye.reshape(-1,32,32,1))[0]
...
def show_performance_of_model_on_eye_dataset(model_path,imgs_dir,left_eye):
    eyeclosed_model = EyeStateModel((32,32,1))
    model = eyeclosed_model.model
...
def eye_state_multiinput_webcam_demo(model_path):
    eyeclosed_model = MultiInputEyeStateModel((32,32,1))
    model = eyeclosed_model.model
    model.load_weights(model_path)
    cap = cv2.VideoCapture(-1)
    detector = dlib.get_frontal_face_detector()
